  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        setErrorMsg('Sign up failed. This email may already be in use.');
        setIsLoading(false);
      }
    } catch {
      setErrorMsg('Registration timeout. Please check your connection.');
      setIsLoading(false);
    }
  };
      ...
      <Card className="max-w-[420px] w-full bg-[#101018] border-white/5 shadow-2xl rounded-2xl overflow-hidden relative">
        ...
        <CardHeader className="p-6 text-center space-y-1">
          ...
          <CardDescription className="text-xs text-gray-400">
            Sign up for your secure digital goods top-up account.
          </CardDescription>
        </CardHeader>

        <form onSubmit={handleSubmit}>
          <CardContent className="px-6 pb-6 space-y-4 text-xs font-sans">
            {errorMsg && (
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">Full Name</label>
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">Password</label>
              <div className="relative font-sans">
          ...
          <CardFooter className="px-6 pb-6 pt-0 flex flex-col gap-4">
            ...
            <p className="text-center text-xs text-gray-500 font-sans">
              ...
              <Link to={`/auth/login?redirect=${encodeURIComponent(redirect)}`} className="text-indigo-400 hover:text-white hover:underline font-semibold font-sans">
                Sign In
              </Link>
            </p>
          </CardFooter>
